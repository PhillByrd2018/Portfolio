...
$sql = "insert into Certificate_Info (ContractID, Address, SSN, DC_Num) values(" .
  $_REQUEST["ID"] . ",'" .
  $_REQUEST["Address"] . "'," .
  $_REQUEST["SSN"] . "," .
  $_REQUEST["DC_Num"] . ")";
...
<h2>You have successfully added a entry to the Database<span style='color:green;'>(Death Certificate Info).</span></h2>
<hr>
<br>
<br>
...
$sql_print = "select * from Certificate_Info where ContractID= '" . $_REQUEST["ID"] . "'";
...
while($row_print = $result_print->fetch_assoc()) {
  ...
  echo '<td>' . $row_print['SSN'] . '</td>';
  echo '<td>' . $row_print['DC_Num'] . '</td>';
  echo "</tr>";
  ...
  $ID = $row_print['ContractID'];
	...
	echo '<br>';
	echo '<hr>';
	echo '<h2> Proceed to fillout additional Cause of Death Information</h2>';
	echo "<a href='addCauseCLT.php?id=" . $ID ."'><button class='btn btn-info'>Cause of Death Info.</button></a>";
