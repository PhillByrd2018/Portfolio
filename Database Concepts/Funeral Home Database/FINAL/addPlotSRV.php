...
<div id="container">
	  <div id="nav">
				...
				<h2 style="text-align: center;vertical-align: middle;line-height: 80px;">Plot Table</h2>
...
$sql = "insert into Plot_Info (ContractID, Section, Plot_Number, Vault, HeadStone) values(" .
  $_REQUEST["ID"] . ",'" .
  $_REQUEST["Section"] . "'," .
  $_REQUEST["Plot_Number"] . ",'" .
  $_REQUEST["Vault"] . "','" .
  $_REQUEST["HeadStone"] . "')";
...
<h2>You have successfully added a entry to the Database<span style='color:green;'>(Plot Info).</span> </h2>
<hr>
<br>
<br>
...
$sql_print = "select * from Plot_Info where ContractID= '" . $_REQUEST["ID"] . "'";
...
while($row_print = $result_print->fetch_assoc()) {
  ...
  $ID = $row_print['ContractID'];
	...
	echo '<br>';
	echo '<hr>';
	echo '<h2>Proceed to fillout additional Death Certificate Info.</h2>';
	echo "<a href='addCertCLT.php?id=" . $ID ."'><button class='btn btn-info'>Death Certificate Info.</button></a>";
