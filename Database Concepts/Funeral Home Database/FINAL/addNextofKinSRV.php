...
$sql = "insert into Next_Of_Kin (ContractID, firstName,lastName, Address, Phone, Email) values('" .
  $_REQUEST["ID"] . "','" .
  $_REQUEST["firstname"] . "','" .
  $_REQUEST["lastname"] . "','" .
  $_REQUEST["Address"] . "'," .
...
<h2>You have successfully added a entry to the Database<span style="color:green;">(Next of Kin)</span>.</h2>
<hr>
<br>
<br>
...
$sql_print = "select * from Next_Of_Kin where ContractID = '" . $_REQUEST["ID"] . "'";
...
while($row_print = $result_print->fetch_assoc()) {
  ...
  echo '<td>' . $row_print['NOK_ID'] . '</td>';
  echo '<td>' . $row_print['firstName'] . '</td>';
  echo '<td>' . $row_print['lastName'] . '</td>';
  echo '<td>' . $row_print['Address'] . '</td>';
  echo '<td>' . $row_print['Phone'] . '</td>';
  echo '<td>' . $row_print['Email'] . '</td>';
  echo "</tr>";
  ...
  $ID = $row_print['ContractID'];
	...
	echo '<br>';
	echo '<hr>';
	echo '<h2>Proceed to fill out services Information</h2>';
	echo "<a href='addServiceCLT.php?id=" . $ID ."'><button class='btn btn-info'>Service Info.</button></a>";
