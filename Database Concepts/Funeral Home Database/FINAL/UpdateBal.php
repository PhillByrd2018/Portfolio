...
<div id="container">
	  <div id="nav">
				...
				<h2 style="text-align: center;vertical-align: middle;line-height: 80px;">Update Balance Procedure</h2>
...
$sql_Procedure = "CALL UpdateBAL(".$_REQUEST["ContractID"].",".$_REQUEST["TotalAMT"].",".$_REQUEST["Paid"].")";



$result_Procedure = myquery($mysqli, $sql_Procedure);
...
<th>Contract ID</th>
<th>Total Amount For Services</th>
<th>Amount Paid</th>
...
$sql_print = "select * from Balances Where ContractID= '" . $_REQUEST["ContractID"] . "'";
...
while($row_print = $result_print->fetch_assoc()) {
  ...
  echo '<td>' . $row_print['TotalAmount'] . '</td>';
  echo '<td>' . $row_print['Paid'] . '</td>';
  echo '<td>' . $row_print['Remaining'] . '</td>';
  echo "</tr>";
...
<a href='database.php'><button class='btn btn-primary'>Return to Database</button></a>
		...
		<div id="footer">
			<img src="cheeztiz.gif" style="position: fixed;left: 0;bottom: 0;height: 40%;">
